...
<script lang="ts">
  ...
  import { formSchema } from './posDailyReport.data';
  import { BasicDrawer, useDrawerInner } from '/@/components/Drawer';
  import { savePosDailyReport, updatePosDailyReport } from '/@/api/pos/posDailyReport';
  import { useI18n } from '/@/hooks/web/useI18n';

  export default defineComponent({
    name: 'PosDailyReportDrawer',
    ...
    setup(_, { emit }) {
      ...
      async function handleSubmit() {
        try {
          ...
          if (!unref(isUpdate)) {
            await savePosDailyReport(values);
          } else {
            values.id = recordId;
            await updatePosDailyReport(values);
          }
