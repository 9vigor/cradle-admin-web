...
<script lang="ts">
  ...
  import { formSchema } from './posComboOrder.data';
  import { BasicDrawer, useDrawerInner } from '/@/components/Drawer';
  import { savePosComboOrder, updatePosComboOrder } from '/@/api/pos/posComboOrder';
  import { useI18n } from '/@/hooks/web/useI18n';

  export default defineComponent({
    name: 'PosComboOrderDrawer',
    ...
    setup(_, { emit }) {
      ...
      async function handleSubmit() {
        try {
          ...
          if (!unref(isUpdate)) {
            await savePosComboOrder(values);
          } else {
            values.id = recordId;
            await updatePosComboOrder(values);
          }
