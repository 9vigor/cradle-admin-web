...
<script lang="ts">
  ...
  import { formSchema } from './posSpendItem.data';
  import { BasicDrawer, useDrawerInner } from '/@/components/Drawer';
  import { savePosSpendItem, updatePosSpendItem } from '/@/api/pos/posSpendItem';
  import { useI18n } from '/@/hooks/web/useI18n';

  export default defineComponent({
    name: 'PosSpendItemDrawer',
    ...
    setup(_, { emit }) {
      ...
      async function handleSubmit() {
        try {
          ...
          if (!unref(isUpdate)) {
            await savePosSpendItem(values);
          } else {
            values.id = recordId;
            await updatePosSpendItem(values);
          }
