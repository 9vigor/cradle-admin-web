...
<script lang="ts">
  ...
  import { formSchema } from './memberWallet.data';
  import { BasicDrawer, useDrawerInner } from '/@/components/Drawer';
  import { saveMemberWallet, updateMemberWallet } from '/@/api/member/memberWallet';
  import { useI18n } from '/@/hooks/web/useI18n';

  export default defineComponent({
    name: 'MemberWalletDrawer',
    ...
    setup(_, { emit }) {
      ...
      async function handleSubmit() {
        try {
          ...
          if (!unref(isUpdate)) {
            await saveMemberWallet(values);
          } else {
            values.id = recordId;
            await updateMemberWallet(values);
          }
