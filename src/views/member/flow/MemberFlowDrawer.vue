...
<script lang="ts">
  ...
  import { formSchema } from './memberFlow.data';
  import { BasicDrawer, useDrawerInner } from '/@/components/Drawer';
  import { saveMemberFlow, updateMemberFlow } from '/@/api/member/memberFlow';
  import { useI18n } from '/@/hooks/web/useI18n';

  export default defineComponent({
    name: 'MemberFlowDrawer',
    ...
    setup(_, { emit }) {
      ...
      async function handleSubmit() {
        try {
          ...
          if (!unref(isUpdate)) {
            await saveMemberFlow(values);
          } else {
            values.id = recordId;
            await updateMemberFlow(values);
          }
