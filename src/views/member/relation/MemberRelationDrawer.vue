...
<script lang="ts">
  ...
  import { formSchema } from './memberRelation.data';
  import { BasicDrawer, useDrawerInner } from '/@/components/Drawer';
  import { saveMemberRelation, updateMemberRelation } from '/@/api/member/memberRelation';
  import { useI18n } from '/@/hooks/web/useI18n';

  export default defineComponent({
    name: 'MemberRelationDrawer',
    ...
    setup(_, { emit }) {
      ...
      async function handleSubmit() {
        try {
          ...
          if (!unref(isUpdate)) {
            await saveMemberRelation(values);
          } else {
            values.id = recordId;
            await updateMemberRelation(values);
          }
